...
#    cl.replicate([data_prep, uvw, wisdom])  # spread data around to search faster
    ...
    for dmind in range(len(st.dmarr)):
        delay = cl.submit(search.calc_delay, st.freq, st.freq[-1], st.dmarr[dmind], st.metadata.inttime, pure=True, workers=workers, allow_other_workers=allow_other_workers)
...
#            candplot = cl.submit(search.candplot, ims_thresh, data_dm)
            feature = cl.submit(search.calc_features, ims_thresh, dmind, st.dtarr[dtind], dtind, segment, st.parameters.features, pure=True, workers=workers, allow_other_workers=allow_other_workers)
